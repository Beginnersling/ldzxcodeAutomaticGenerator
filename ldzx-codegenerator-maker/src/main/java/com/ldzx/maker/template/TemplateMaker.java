...
import com.ldzx.maker.template.model.TemplateMakerConfig;
...
public class TemplateMaker {
    ...
    // 三、生成配置文件
        ...
        if(templateMakerOutputConfig != null){
            //文件外层和分组去重
            if(templateMakerOutputConfig.isRemoveGroupFilesFormRot()){
                List<Meta.FileConfig.FileInfo> fileInfoList = newMeta.getFileConfig().getFiles();
...
//        String searchStr = "Sum: ";
        ...
        long id = makeTemplate(meta, originProjectPath, templateMakerFileConfig, templateMakerModelConfig,null, null);
        System.out.println(id);
        //todo 删除id的文件后再次生成不会出现相关文件
    }
}